...
const SearchData = () => {
    ...
    const { setResults, setResultst, searchMovies, searchTv, setQuery, resultst, results, tv, movies } = StateG()
    setQuery(query)
    ...
    const searchData = <div className=' max-[428px]:flex-col gap-[1rem] flex w-[100%] p-[1rem] ' >
        <div className=' max-[428px]:w-[100%] w-[25%] flex flex-col h-[15rem] rounded-lg border-[1px] ' >
            ...
            <div className=' flex flex-col gap-[1rem] py-[1rem] h-auto   text-[#111111] ' >
                <div
                style={{
                    backgroundColor:isMovie? "#64748b80":''
                }}
                    onClick={() => {
                        setIsMovie(true)
        ...
        <div className=' flex h-full max-h-full overflow-scroll flex-col p-[1rem] gap-[1rem] max-[428px]:w-[100%] w-[75%] ' >
            {
                results?.map(data => {
                    return (
                        data ? (
            ...
            }
            {
                resultst?.map(data => {
                    return (
                        data ? (
                            <div key={data.id} className=' w-[100%] border-[1px] shadow-lg rounded-lg h-auto gap-[1rem] flex ' >
                                <Link to={`/overeview/${data.id}`}>
                                    <img loading='lazy' className=' blurc object-cover rounded-l-lg min-w-[94px] h-[141px] ' src={`http://image.tmdb.org/t/p/w500/${data.poster_path}`} alt="" />
